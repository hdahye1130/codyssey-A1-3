...
RECOMMENDATION = object_schema(['title', 'creator', 'country', 'reason', 'why_match'])
SCHEMA = {
    'type': 'object',
    'properties': {
        'pattern': {
            ...
            'properties': {
                ...
                **{key: text_schema() for key in ['summary', 'connection', 'mood', 'detail', 'feeling', 'one_line']},
            },
            'required': ['keywords', 'summary', 'connection', 'mood', 'detail', 'feeling', 'one_line'],
        },
...
def mock_result(things):
    ...
    def rec(title, creator='', country=''):
        return {'title': title, 'creator': creator, 'country': country, 'reason': '익숙한 취향을 다른 분야로 확장하는 예시 추천이에요.', 'why_match': f'{first}와 {second} 사이의 분위기에서 출발했어요.'}
    return {
        'pattern': {'keywords': ['여백', '잔잔한 대비', '발견', '겹침'], 'summary': '서로 다른 장면이 겹쳐 새로운 분위기를 만듭니다.', 'connection': f'{first}와 {second}을(를) 비롯한 취향을 나란히 놓으면, 익숙한 것 속에서 새로운 감각을 발견하려는 흐름이 보입니다. 이 문장은 mock 예시이며 AI 분석이 아닙니다.', 'mood': '부드럽고 열린 분위기', 'detail': '작은 차이를 오래 바라보는 시선', 'feeling': '천천히 발견하는 즐거움', 'one_line': '익숙한 것들의 틈에서 새로운 결을 발견하는 취향.'},
...
def analyze(things):
    ...
    prompt = (
        ...
        'GO의 country를 포함하세요. 나머지 추천의 country는 빈 문자열이어도 됩니다. '
        '\n취향 데이터: '
