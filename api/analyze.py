...
IMAGE_SEARCH_RECOMMENDATION = object_schema([*RECOMMENDATION['required'], 'image_search_title', 'image_search_creator'])
SCHEMA = {
    'type': 'object',
    'properties': {
        'pattern': {
        ...
        'discover': {'type': 'object', 'properties': {key: IMAGE_SEARCH_RECOMMENDATION if key in ('listen', 'read') else RECOMMENDATION for key in ['watch', 'listen', 'read', 'go']}, 'required': ['watch', 'listen', 'read', 'go']},
    },
...
def mock_result(things):
    ...
    return {
        ...
        'discover': {'watch': rec('리틀 포레스트', '임순례'), 'listen': {**rec('Vespertine', 'Björk'), 'image_search_title': 'Vespertine', 'image_search_creator': 'Björk'}, 'read': {**rec('모모', '미하엘 엔데'), 'image_search_title': 'Momo', 'image_search_creator': 'Michael Ende'}, 'go': rec('교토 철학의 길', country='일본')},
    }
...
def analyze(things):
    ...
    prompt = (
        ...
        'LISTEN과 READ의 image_search_title / image_search_creator에는 각각 iTunes / Open Library 표지 검색에 적합한 공식 원제 또는 영문 제목과 아티스트 / 저자의 공식 원어 또는 영문 이름을 가능하면 넣으세요. 표시용 title / creator와 같은 작품 / 인물이어야 하며, 확실하지 않은 검색 필드는 빈 문자열로 두세요. '
