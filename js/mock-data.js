...
window.createMoirePreview = function (things) {
    ...
    const recommendation = (title, creator, reason, whyMatch, country = '') => ({
        title, creator, country, reason, why_match: whyMatch
    });
    return {
        pattern: {
        ...
        discover: {
            ...
            go: recommendation('교토 철학의 길', '', '물길을 따라 천천히 걸으며 풍경을 볼 수 있는 장소예요.', '작은 변화에 시선을 두는 예시 패턴을 공간으로 넓혀줘요.', '일본')
        }
    };
};
