...
3. 로컬 실제 API 테스트는 `.env.local`의 `GEMINI_API_KEY=` 뒤에 본인의 키를 입력하세요. 키를 HTML, JavaScript, Python 소스에 직접 넣거나 Git에 커밋하면 안 됩니다. 필요하면 `GEMINI_MODEL`을 지정할 수 있으며 기본값은 `gemini-2.5-flash`입니다.
...
정적 HTML 파일만 열면 입력과 목록은 작동하지만 `/api/analyze` 요청은 Vercel 개발 서버 또는 배포 환경이 필요합니다. 실제 이미지 API는 아직 연결되지 않았고, 추천 카드에는 모아레 fallback visual이 표시됩니다. AI 응답의 `image_prompt`는 이후 별도 이미지 생성 엔드포인트에 연결할 수 있습니다.
